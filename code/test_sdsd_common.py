...
from sde_lowlight_data import image_from_tensor, tensor_from_image
from sde_lowlight_model import LowLightEnhanceNet
from sdsd_lowlight_data import (
    SDSD_DEFAULT_ROOT,
    collect_sdsd_pairs,
    event_tensor_from_npz,
)
from train_sde_common import ssim_index


def parse_args(default_subset=None):
    parser = argparse.ArgumentParser(description="Run SDSD event low-light enhancement inference.")
    ...
    if args.subset is None and args.input_dir is None:
        parser.error("--subset is required when using test_sdsd_common.py directly")
    if args.checkpoint is None and args.subset is not None:
        args.checkpoint = f"checkpoints/sdsd_{args.subset}/best.pt"
    if args.output_dir is None:
        name = args.subset if args.subset is not None else "custom"
        args.output_dir = f"results/sdsd_{name}_{args.split}"
...
def load_model(args, device):
    ...
    event_bins = int(saved_args.get("event_bins", args.event_bins))
...
def collect_custom_inputs(input_dir):
    ...
    paths = sorted(path for path in root.rglob("*.npz") if path.is_file())
    return [
        {
            "low": path,
            "normal": None,
            "scene": path.parent.name,
            "low_name": f"{path.stem}.png",
        }
        for path in paths
    ]
...
def _sample_size(sample, args):
    if sample["normal"] is not None:
        with Image.open(sample["normal"]) as normal_image:
            width, height = normal_image.size
        return height, width
    if args.image_height is None or args.image_width is None:
        raise ValueError(
            "--image-height and --image-width are required when --input-dir has no normal images"
        )
...
def run_inference(args):
    ...
    model, event_bins = load_model(args, device)
    ...
    if args.input_dir:
        ...
        samples = collect_sdsd_pairs(args.data_root, args.subset, args.split, require_gt=False)
    ...
    for index, sample in enumerate(samples, start=1):
        height, width = _sample_size(sample, args)
        low = event_tensor_from_npz(sample["low"], height, width, event_bins)
